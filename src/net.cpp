...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "tempcoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"tempcoin.com", "172.30.1.9"},
    {NULL, NULL}
};

static const char *strTestNetDNSSeed[][2] = {
    {"tempcointools.com", "testnet-seed.tempcointools.com"},
...
void ThreadOpenConnections()
{
    ...
    loop
    {
        ...
        if (addrman.size()==0 && (GetTime() - nStart > 60) && !fTestNet)
        {
            ...
            addrman.Add(vAdd, CNetAddr("172.30.1.9"));
        }
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. tempcoin is probably already running."), addrBind.ToString().c_str());
        else
...
void StartNode(boost::thread_group& threadGroup)
{
    ...
    if (pnodeLocalHost == NULL)
        pnodeLocalHost = new CNode(INVALID_SOCKET, CAddress(CService("172.30.1.9", 0), nLocalServices));
