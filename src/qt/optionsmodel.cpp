...
{
    QSettings settings;
    CService addrProxy(settings.value("addrProxy", "172.30.1.9:9050").toString().toStdString());
    ...
    if(role == Qt::EditRole)
    {
        ...
        case ProxyIP: {
            ...
            if (GetProxy(NET_IPV4, proxy))
                ...
                return QVariant(QString::fromStdString("172.30.1.9"));
        }
        case ProxyPort: {
            proxyType proxy;
            if (GetProxy(NET_IPV4, proxy))
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case ProxyIP: {
            proxyType proxy;
            proxy.first = CService("172.30.1.9", 9050);
