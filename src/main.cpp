...
#if defined(NDEBUG)
# error "tempcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x479563ecaab7bd878d7b75f6192d8e956c0eacafcca018b694c2e0ae68c21a14");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // tempcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "tempcoin Signed Message:\n";
...
{
    // tempcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // tempcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // tempcoin: 3.5 days
static const int64 nTargetSpacing = 1 * 30; // tempcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // tempcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // tempcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x479563ecaab7bd878d7b75f6192d8e956c0eacafcca018b694c2e0ae68c21a14");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is TempCoin by Silvercolor at 2018-06-01";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("123584710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1527845006;
        ...
        assert(block.hashMerkleRoot == uint256("0x9e491e5c3a44ef886ba432ee6a3e43204ccb2a5b56ad63ed66704eaf7bb7d386"));

        // If genesis block hash does not match, then generate new genesis hash.
        printf("\n Added source area start... \n");
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // tempcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("tempcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("tempcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static tempcoinMiner(CWallet *pwallet)
{
    printf("tempcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tempcoin-miner");
    ...
    try { loop {
        ...
        printf("Running tempcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("tempcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&tempcoinMiner, pwallet));
}
